...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!isset($_SESSION['tipo_entrega'])) {
        echo "Tipo de entrega não selecionado.";
        exit;
    }
    $tipo_entrega = $_SESSION['tipo_entrega'];  // Recupera o tipo de entrega da sessão
    ...
    try {
        ...
        // Inserção do pedido (incluindo tipo de entrega)
        $stmt = $conn->prepare("INSERT INTO pedidos (cliente_id, total, tipo_entrega) VALUES (?, ?, ?)");
        $stmt->bind_param("ids", $cliente_id, $total, $tipo_entrega);
        $stmt->execute();
        ...
        // Limpar o carrinho e o tipo de entrega da sessão
        $_SESSION['carrinho'] = [];
        unset($_SESSION['tipo_entrega']);  // Limpar a sessão após o pedido ser finalizado
